...
class Postcomment(APIView):
    ...
    def post(self, request, pk):
        ...
        data = {"post": pk, "comment": comment_text}
...
class PostLikeDislikeView(APIView):
    ...
    def post(self, request, pk):
        ...
        post_interaction, created = Postinteraction.objects.get_or_create(
            post=post, liked_disliked_by=request.user
        )
        data = request.data
        ...
        if data.get("liked") is not None:
            ...
            post_interaction.disliked = (
                False if data["liked"] else post_interaction.disliked
            )
        elif data.get("disliked") is not None:
            post_interaction.disliked = data["disliked"]
            post_interaction.liked = (
                False if data["disliked"] else post_interaction.liked
            )

        post_interaction.save()

        return Response(
            {"liked": post_interaction.liked, "disliked": post_interaction.disliked}
        )
...
class Mostlikedpost(APIView):
    def get(self, request):
        posts = Post.objects.annotate(
            likes_count=Count(
                "postinteraction", filter=models.Q(postinteraction__liked=True)
            )
